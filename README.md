...
### Папка "!base"
Специальная папка с префиксом "!" для хранения базовых заметок. Эта папка всегда будет отображаться в начале списка файлов.

Содержит:
- `_Личное.md` - заметка с личной информацией
- `Блокнот.md` - заметка для быстрых записей
